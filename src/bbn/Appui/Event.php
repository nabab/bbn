...
class Event extends modelDb
{
  ...
  private $recurrences = ["daily,", "weekly", "monthly", "yearly"];
  private $weekdays = [
    1 => "monday",
    2 => "tuesday",
    3 => "wednesday",
  ...
  public function insert(array $event): ?string
  {
    $f = &$this->fields;
    if (
      X::hasProps($event, [$f["id_type"]], true) &&
      array_key_exists($this->fields["start"], $event)
    ) {
      if (!empty($event[$f["cfg"]]) && !Str::isJson($event[$f["cfg"]])) {
      ...
      if ($is_rec = !empty($event[$f["recurring"]])) {
        ...
        if (
          ...
          if (!empty($event[$f["end"]])) {
            $diff = date_diff(
              new DateTime($event[$f["start"]]),
              new DateTime($event[$f["end"]]),
            );
      ...
      if (
        ...
        if ($is_rec) {
          $this->db->insert($this->class_cfg["tables"]["recurring"], [
            ...
            $rf["wd"] => !empty($event[$rf["wd"]])
              ? (Str::isJson($event[$rf["wd"]])
                ? $event[$rf["wd"]]
                : json_encode($event[$rf["wd"]]))
              : null,
            $rf["mw"] => !empty($event[$rf["mw"]])
              ? (Str::isJson($event[$rf["mw"]])
                ? $event[$rf["mw"]]
                : json_encode($event[$rf["mw"]]))
              : null,
            $rf["md"] => !empty($event[$rf["md"]])
              ? (Str::isJson($event[$rf["md"]])
                ? $event[$rf["md"]]
                : json_encode($event[$rf["md"]]))
              : null,
            $rf["ym"] => !empty($event[$rf["ym"]])
              ? (Str::isJson($event[$rf["ym"]])
                ? $event[$rf["ym"]]
                : json_encode($event[$rf["ym"]]))
              : null,
          ]);
        }
        return $id;
  ...
  public function getRecurrences(
    ...
    // Get occurrences
    $occ = $this->makeRecurrencesFields(
      $event,
      $when->getOccurrencesBetween(new \DateTime($start), new \DateTime($end)),
    );
  ...
  public function makeRecurrencesFields(array $event, array $recurrences): array
  {
    ...
    $diff = !empty($event[$ef["end"]])
      ? date_diff(
        new DateTime($event[$ef["start"]]),
        new DateTime($event[$ef["end"]]),
      )
      : false;
    // Fix fields
   ...
   * @return string|null
   */
  public function getFirstRecurrence(
    array $event,
    $omitstart = true,
    $exceptions = false,
  ): ?string {
    if (
      ...
      $t = &$this;
      $event[$this->class_cfg["extra"]["exceptions"]] = array_map(function (
        $e,
      ) use ($t) {
        return $e[$t->class_cfg["arch"]["exceptions"]["day"]] .
          " " .
          $e[$t->class_cfg["arch"]["exceptions"]["start"]];
      }, $excs);
    }
    $when = $this->getWhenObject($event);
  ...
  {
    if (Str::isUid($id)) {
      $todelete = $this->db->count($this->class_cfg["tables"]["recurring"], [
        $this->class_cfg["arch"]["recurring"]["id_event"] => $id,
      ]);
      return $this->db->delete($this->class_cfg["tables"]["recurring"], [
        $this->class_cfg["arch"]["recurring"]["id_event"] => $id,
      ]) === $todelete;
    }
  ...
  public function filterRecurrencesByExceptions(array $recurrences): array
  {
    if (
      ...
      return array_filter($recurrences, function ($r) use ($ex, $ef, $exf) {
        return X::search($ex, [
          $exf["day"] => date("Y-m-d", strtotime($r[$ef["start"]])),
        ]) === null;
      });
    }
    return $recurrences;
  ...
  {
    if (Str::isUid($id) && (\is_null($until) || Str::isDateSql($until))) {
  ...
  public function addException(string $id_event, array $exc): bool
  {
    if (
      ...
      !empty($exc[$exf["start"]]) &&
      !empty($exc[$exf["end"]]) &&
      (!empty($exc[$exf["deleted"]]) || !empty($exc[$exf["rescheduled"]]))
    ) {
      ...
      $exc[$exf["id_user"]] = !empty($exc[$exf["id_user"]])
        ? $exc[$exf["id_user"]]
   ...
   * @return array
   */
  protected function filterRecurrencesByMonthWeek(
    array $recurrences,
    int $monthweek,
  ): array {
    $t = &$this;
    ...
    // Events table fields
    $ef = &$this->class_cfg["arch"]["events"];
    return array_filter($recurrences, function ($o) use (
      $ef,
      $rf,
      $t,
      $monthweek,
      ...
      if ($monthweek < 0) {
        ...
        $lwd = Date::getMonthWeek(
          date(
            "Y-m-d",
            strtotime("-" . abs($monthweek + 1) . " week", $lastwd),
          ),
        );
        return $lwd === $week;
      }
      // From the start
        ...
        // Get the corrected week number
        $fwd = Date::getMonthWeek(
          date("Y-m-d", strtotime("+" . ($monthweek - 1) . " week", $firstwd)),
        );
        return $fwd === $week;
      }
    });
