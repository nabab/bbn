...
class Option extends bbn\Models\Cls\Db
{
  ...
  public function getValue($code = null): ?array
  {
    if (bbn\Str::isUid($id = $this->fromCode(\func_get_args()))
        ...
        && bbn\Str::isJson($opt[$this->fields['value']])
    ) {
   ...
   */
  public function findPermissions($id = null, bool $deep = false, bool $force = false)
  {
    if ($this->check()) {
      if (\is_null($id)) {
        $id = $this->default;
      ...
      $cfg = $this->getCfg($id);
      if ($force || !empty($cfg['permissions'])) {
        if (!$force && in_array($cfg['permissions'], ['all', 'cascade'])) {
          $force = true;
        }
        $perms = [];
        if ($opts  = $this->fullOptions($id)) {
          foreach ($opts as $opt){
            $o = [
              'icon' => $opt[$this->fields['cfg']]['icon'] ?? 'nf nf-fa-key',
              'text' => $opt[$this->fields['text']],
              'id' => $opt[$this->fields['id']]
            ];
            if ($deep) {
              $o['items'] = $this->findPermissions($opt[$this->fields['id']], true, $force);
            }
  ...
  private function _prepare(array &$it): bool
  {
    ...
    if (isset($it[$c['cfg']])) {
      if (!is_array($it[$c['cfg']]) && bbn\Str::isJson($it[$c['cfg']])) {
