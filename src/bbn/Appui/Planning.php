...
class Planning extends DbCls
{
  ...
  private function _delete(string $id_planning, ?string $id_event = null): bool
  {
    ...
    ]) && (bool) $this->events->delete($id_event);
  }
  ...
  public function insert(
    ...
    if (
      ...
      $this->db->insert($this->class_table, [
        ...
        $this->fields["alias"] => \bbn\Str::isDateSql($alias)
          ? date("Y-m-d", strtotime($alias))
          : null,
      ])
    ) {
  ...
  public function update(
    ...
    if (
      ...
      (\bbn\Str::isUid($event) ||
        (\is_array($event) &&
          \bbn\Str::isUid($event[$this->ecfg["arch"]["events"]["id"]]))) &&
      (\bbn\Str::isUid($id_alias) || \is_null($id_alias)) &&
      (\bbn\Str::isDateSql($alias) || \is_null($alias))
    ) {
      $id_event = \is_array($event)
        ? $event[$this->ecfg["arch"]["events"]["id"]]
        : $event;
      $ok = $this->db->update(
        $this->class_table,
        [
        ...
        switch ($event[$extf["action"]]) {
          case "this":
            ...
            if (!empty($old_event[$ef["recurring"]])) {
              ...
              if (!empty($event[$extf["recurrence"]])) {
                ...
                ($first_recc = $this->events->getFirstRecurrence(
                  $old_event,
                  true,
                  true,
                )) &&
                // Make the recurrences fields structure
                ($event_next = $this->events->makeRecurrencesFields(
                  $old_event,
                  [$first_recc],
                ))
              ) {
          ...
          case "future":
            if (!empty($event[$extf["recurrence"]])) {
              $until = date(
                "Y-m-d",
                strtotime("-1 day", strtotime($event[$ef["start"]])),
              );
  ...
  {
    $t = &$this;
    return (bool) array_filter($this->getAllByAlias($id_planning), function (
      $a,
    ) use ($day, $t) {
