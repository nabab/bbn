...
class Notification extends DbCls
{
  ...
  private $opt, $user, $pref, $perms, $cfg, $lastDbId, $lastId;
  ...
  public function create(
    ...
    if ($list_opt = self::getOptionId("list")) {
      ...
      $perms =
        \is_bool($perms) && !empty($perms) && defined("BBN_ID_PERMISSION");
      if (!($id_opt = $this->opt->fromPath($opt_path, "/", $list_opt))) {
        ...
        if (count($bits) === 2) {
          if ($perms) {
            ...
            $is_public = (bool) $this->opt->getProp(
              BBN_ID_PERMISSION,
              "public",
            );
                  ...
                  foreach ($permissions as $perm) {
                    ...
                    if (
                      $this->db->insert($pcfg["table"], [
                        ...
                        $pcfg["arch"]["user_options"]["id_user"] =>
                          $perm->{$pcfg["arch"]["user_options"]["id_user"]},
                        $pcfg["arch"]["user_options"]["id_group"] =>
                          $perm->{$pcfg["arch"]["user_options"]["id_group"]},
                      ])
                    ) {
      ...
      if (Str::isUid($id_opt)) {
        if ($perms) {
          return $perms
            ? $this->insertByOption($title, $content, $id_opt, $user_excluded)
            : $this->insert($title, $content, $id_opt, $users, $user_excluded);
        }
      }
    }
  ...
  public function insert(
    ...
    if (
      !empty($title) &&
      !empty($content) &&
      (\is_null($id_option) || Str::isUid($id_option))
    ) {
      $notification = [
      ...
      $this->lastDbId = $this->db->lastId();
      if (
        $this->db->insert($this->class_cfg["tables"]["content"], $notification)
      ) {
        $id = $this->db->lastId();
        ...
        foreach ($users as $u) {
          if (
            (!$user_excluded || $current_id_user !== $u) &&
  ...
  public function insertByOption(
    string $title,
    string $content,
    string $id_option,
    bool $user_excluded = false,
    ...
    if (
      ...
      ($ocfg = $this->opt->getClassCfg()) &&
      ($groups = $this->db->getColumnValues(
        $ucfg["tables"]["groups"],
        $ucfg["arch"]["groups"]["id"],
        [$ucfg["arch"]["groups"]["type"] => "real"],
      )) &&
      ($id_perm = $this->db->selectOne(
        $ocfg["table"],
        $ocfg["arch"]["options"]["id"],
        [$ocfg["arch"]["options"]["code"] => "opt" . $id_option],
      )) &&
  ...
  {
    if (Str::isUid($id)) {
      return (bool) $this->db->delete($this->class_table, [
        $this->fields["id"] => $id,
      ]);
    }
  ...
  public function read($id, string|null $id_user = null, $moment = null): bool
  {
    ...
    if (Str::isUid($id_user)) {
      if (\is_array($id)) {
        ...
        Str::isUid($id) &&
        !$this->db->selectOne($this->class_table, $this->fields["read"], [
          $this->fields["id"] => $id,
        ])
      ) {
        return (bool) $this->db->update(
          $this->class_table,
          [
            $this->fields["read"] => $moment
              ? \round((float) $moment, 4)
              : X::microtime(),
          ],
          [
            $this->fields["id"] => $id,
  ...
  public function readAll(string|null $id_user = null, $moment = null): bool
  {
    ...
    if (Str::isUid($id_user) && ($unreads = $this->getUnreadIds($id_user))) {
      $todo = count($unreads);
      $did = 0;
              ...
              "conditions" => [
                [
                  "field" => $this->db->colFullName(
                    $this->fields["id_content"],
                    $this->class_table,
                  ),
                  "exp" => $this->db->colFullName(
                    $this->class_cfg["arch"]["content"]["id"],
                    $this->class_cfg["tables"]["content"],
                  ),
                ],
              ],
            ],
  ...
  public function notify($notification): ?bool
  {
    ...
    if (
      ...
      ($cfg = $this->getCfg(
        $id_user,
        $notification[$this->class_cfg["arch"]["content"]["id_option"]],
      ))
    ) {
      $mtime = X::microtime();
      ...
      $sessions = $this->db->selectAll(
        ...
        [$ucfg["arch"]["sessions"]["id"], $ucfg["arch"]["sessions"]["sess_id"]],
        [
      ...
      // Web notification
      if (
        empty($notification[$this->fields["web"]]) &&
        !empty($cfg["web"]) &&
        !empty($sessions) &&
        ...
        foreach ($sessions as $sess) {
          ...
          if (Dir::createPath($path) && !\is_file($path . "$mtime.json")) {
            ...
            $notification[$this->fields["dt_web"]] = date(
              "Y-m-d H:i:s",
              $mtime,
            );
            file_put_contents(
      ...
      elseif (
        empty($notification[$this->fields["browser"]]) &&
        !empty($cfg["browser"]) &&
        !empty($sessions) &&
        empty($notification[$this->fields["mail"]])
      ) {
        foreach ($sessions as $sess) {
          ...
          if (Dir::createPath($path) && !\is_file($path . "$mtime.json")) {
            ...
            $notification[$this->fields["dt_browser"]] = date(
              "Y-m-d H:i:s",
              $mtime,
            );
            file_put_contents(
      ...
      elseif (
        ...
        $creation = strtotime(
          $notification[$this->class_cfg["arch"]["content"]["creation"]],
        );
        if (
          $cfg["mail"] === "immediately" ||
          ($cfg["mail"] === "daily" &&
  ...
  public function getCfg(string $id_user, string|null $id_option = null): ?array
  {
    if (
      ...
      if (
        ...
        ($not_parent_pref = $this->pref->getCfgByOption(
          $id_option_parent,
          $id_user,
        ))
      ) {
  ...
  public function setCfg(array $cfg): bool
  {
    if (
      ...
      return (bool) $this->pref->updateByOption($cfg["id_option"], [
        "web" => (bool) $cfg["web"],
  ...
  private function _update(string $id, array $notification): ?bool
  {
    if (Str::isUid($id) && !empty($notification)) {
      ...
      $f = $this->fields;
      $fields = array_values(
        array_filter($this->fields, function ($field) use ($f) {
          return $field !== $f["id"] &&
            $field !== $f["dt_web"] &&
            $field !== $f["dt_browser"] &&
            $field !== $f["dt_mail"] &&
  ...
  private function _send_mail(string $id_user, array $notifications): ?bool
  {
    if (
      ...
      $templ["title"] = str_replace(
        "{{app_name}}",
        defined("BBN_SITE_TITLE")
          ? constant("BBN_SITE_TITLE")
          : constant("BBN_CLIENT_NAME"),
        $templ["title"],
      );
      $this->lastDbId = $this->db->lastId();
      $ret = (bool) $this->db->insert("bbn_emails", [
        "email" => $email,
        "subject" => $templ["title"],
        "text" => $rendered,
      ]);
    ...
    if (
      ...
      foreach ($notis as $n) {
        if (
          ($cfg = $this->getCfg(
            $id_user,
            $n[$this->class_cfg["arch"]["content"]["id_option"]] ?? null,
          )) &&
          !empty($cfg["mail"]) &&
          $cfg["mail"] === $mail_cfg
        ) {
          $n[$this->class_cfg["arch"]["content"]["creation"]] = date(
            "d/m/Y H:i",
            strtotime($n[$this->class_cfg["arch"]["content"]["creation"]]),
          );
          $notifications[] = $n;
  ...
  public function _user_has_permission(
    $notification,
    string|null $id_user = null,
  ): bool {
    if (
    ...
    if (
      ...
      ($id_user =
        $id_user ?:
        $notification[$this->fields["id_user"]] ?? $this->user->getId()) &&
      Str::isUid($id_user) &&
      ($ucfg = $this->user->getClassCfg()) &&
      ($pcfg = $this->pref->getClassCfg()) &&
      ($user = $this->db->select(
      ...
      $id_opt =
        $notification[$this->class_cfg["arch"]["content"]["id_option"]] ?? null;
      if (
        !empty($user->{$ucfg["arch"]["users"]["admin"]}) ||
        !empty($user->{$ucfg["arch"]["users"]["dev"]}) ||
        empty($id_opt)
