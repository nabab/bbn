...
class Uauth extends DbCls
{
  ...
  public function __construct(Db $db, array|null $cfg = null)
  {
    ...
    if (!empty($this->class_cfg["uauth_phone_region"])) {
      $this->class_cfg["uauth_phone_region"] = strtoupper($this->class_cfg["uauth_phone_region"]);
    } elseif (defined(BBN_LOCALE)) {
      $st = explode(".", BBN_LOCALE)[0];
      $this->class_cfg["uauth_phone_region"] = strtoupper(Str::sub($st, -2));
    }
