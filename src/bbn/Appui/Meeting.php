...
class Meeting extends DbCls
{
  ...
  public function __construct(\bbn\Db $db)
  {
    ...
    if (!empty($pref)) {
      ...
      if (!$prefCfg) {
        throw new Exception(
          _("No configuration found for the Preferences class"),
        );
      }
  ...
  public function getRoom(string $idRoom): ?array
  {
    ...
    if (
      ...
      $room = \array_merge(
        $room,
        \json_decode($room[$this->prefFields["cfg"]], true),
      );
  ...
  public function getRooms(
    string $server,
    string|null $idUser = null,
    string|null $idGroup = null,
  ) {
    if (!Str::isUid($server)) {
    ...
    if (!empty($rooms)) {
      foreach ($rooms as $i => $r) {
        $rooms[$i]["moderators"] = $this->getModerators(
          $r[$this->prefFields["id"]],
        );
        if (Str::isJson($r[$this->prefFields["cfg"]])) {
          $rooms[$i] = \array_merge(
            $rooms[$i],
            \json_decode($r[$this->prefFields["cfg"]], true),
          );
  ...
  public function getAllRooms(
    ...
    if (!empty($servers)) {
      ...
      if (!empty($idUser)) {
        ...
        $t = $this;
        if (
          $roomsInvited = $this->db->rselectAll([
            "table" => $this->prefTable,
            ...
            "join" => [
              [
                ...
                "on" => [
                  "conditions" => [
                    [
                      "field" => $this->db->colFullName(
                        $meetingsFields["id_room"],
                        $meetingsTable,
                      ),
                      "exp" => $this->db->colFullName(
                        $this->prefFields["id"],
                        $this->prefTable,
                      ),
                    ],
          ...
          foreach ($roomsInvited as $r) {
            if (
              X::search($rooms, [
                $this->prefFields["id"] => $r[$this->prefFields["id"]],
              ]) === null
            ) {
              $rooms[] = $r;
            }
      ...
      if (!empty($rooms)) {
        foreach ($rooms as $i => $r) {
          ...
          if (
            $idMeeting = $this->getStartedMeeting($r[$this->prefFields["id"]])
          ) {
          ...
          $r["last"] = !empty($last)
            ? $last[$this->class_cfg["arch"]["meetings"]["started"]]
            : "";
          $rooms[$i] = $r;
        }
      }
  ...
  public function getUserRooms(string $server, string $idUser)
  {
    ...
    if (empty($idGroup)) {
      throw new Exception(
        sprintf(_("No group found for the user %s"), $idUser),
      );
    }
  ...
  public function addRoom(
    ...
    if (
      $this->db->selectOne($this->prefTable, $this->prefFields["id"], [
    ...
    if (
      $this->db->insert($this->prefTable, [
        $this->prefFields["text"] => $name,
        $this->prefFields["id_option"] => $idServer,
        $this->prefFields["id_user"] => $idUser,
        $this->prefFields["id_group"] => $idGroup,
        $this->prefFields["public"] =>
          empty($idUser) && empty($idGroup) ? 1 : 0,
  ...
  public function editRoom(
    ...
    if ($old[$this->prefFields["text"]] !== $name) {
      if (
        $this->db->selectOne($this->prefTable, $this->prefFields["id"], [
          [$this->prefFields["text"], "=", $name],
          [
            $this->prefFields["id_option"],
            "=",
            $old[$this->prefFields["id_option"]],
          ],
          [$this->prefFields["id"], "!=", $idRoom],
        ])
      ) {
    ...
    if (
      !empty($toUpd) &&
      !$this->db->update($this->prefTable, $toUpd, [
        $this->prefFields["id"] => $idRoom,
      ])
    ) {
      return false;
      ...
      foreach ($oldModerators as $m) {
        if (!$this->removeModerator($m, $idRoom)) {
          throw new Exception(
            sprintf(
              _(
                "Error during the elimination of the moderator %s from the room %s",
              ),
              $m,
              $idRoom,
            ),
        ...
        if (!$this->addModerator($m, $idRoom)) {
          throw new Exception(
            sprintf(
              _(
                "Error during the insertion of the moderator %s to the room %s",
              ),
              $m,
              $idRoom,
            ),
  ...
  {
    if (Str::isUid($idRoom)) {
      return $this->db->delete($this->prefTable, [
        $this->prefFields["id"] => $idRoom,
      ]);
    }
    return null;
  ...
  public function addModerator(string $idUser, string $idRoom): bool
  {
    if (Str::isUid($idUser) && Str::isUid($idRoom)) {
      if (
        ...
        throw new Exception(
          sprintf(_("No room found with the id %s"), $idRoom),
        );
      }
      ...
      if (
        $this->db->insert($this->prefTable, [
          $this->prefFields["id_option"] =>
            $room[$this->prefFields["id_option"]],
  ...
  public function removeModerator(string $idUser, string $idRoom): bool
  {
    if ($mod = $this->getModerator($idUser, $idRoom)) {
      ...
      return (bool) $this->db->delete($this->prefTable, [
        $this->prefFields["id"] => $idModerator,
      ]);
    }
    return false;
  }
    ...
    if (
      ...
      if (empty($t)) {
        if (!($server = $this->getServerByRoom($idRoom))) {
          throw new Exception(
            sprintf(_("No server found by the room %s"), $idRoom),
          );
        }
        if (
          !$this->passCls->userStore(
            $this->makeJWT(
              $server[$this->optFields["code"]],
              $r[$this->prefFields["text"]],
            ),
            $idModerator,
            $u,
          )
          ...
          throw new Exception(
            sprintf(
              _("Error during JWT storing for the user %s for the room %s"),
              $idUser,
              $idRoom,
            ),
          );
  ...
  public function getModerators(string $idRoom, bool $full = false): ?array
  {
    ...
    $fields = [$this->db->colFullName($userFields["id"], $userCfg["table"])];
  ...
  public function setLeaved(
    ...
    if (
      $this->db->update(
        $this->class_cfg["tables"]["participants"],
        [
  ...
  public function setParticipantName(
    string $idMeeting,
    string $idTmp,
    string $name,
  ): bool {
    ...
    if ($meeting = $this->getMeeting($idMeeting)) {
      ...
      if (empty($old) && ($idRoom = $meeting[$fieldsMeetings["id_room"]])) {
  ...
  public function startMeeting(string $idRoom): string
  {
    ...
    if (
      ...
      throw new Exception(
        sprintf(_("Error starting the meeting for the room %s"), $idRoom),
      );
    }
    return $this->db->lastId();
  }
  ...
  public function stopMeeting(string $idMeeting): bool
  {
    ...
    if (empty($m)) {
      throw new Exception(
        sprintf(_("Meeting not found with the id %s"), $idMeeting),
      );
    }
    $date = date("Y-m-d H:i:s");
    $fields = $this->class_cfg["arch"]["meetings"];
    if (
      \is_null($m[$fields["ended"]]) &&
      !$this->db->update(
      ...
      throw new Exception(
        sprintf(_("Error ending the meeting with the id %s"), $idMeeting),
      );
    }
    $fields = $this->class_cfg["arch"]["participants"];
    $this->db->update(
    ...
    if (
      ...
      foreach ($parts as $part) {
        $name = !empty($part[$partsFields["id_user"]])
          ...
          : (!empty($part[$partsFields["name"]])
            ? sprintf("%s (%s)", $part[$partsFields["name"]], _("External"))
            : _("Unknown external user"));
        $logs[] = [
      ...
      if (!empty($tmpMeetings)) {
        foreach ($tmpMeetings as $meet) {
          if ($m = X::curl($url . "/" . $meet["id"], null, [])) {
            ...
            if (!empty($m["contents"])) {
              $conn = \array_values(
                \array_filter($m["contents"], function ($c) {
                  return \array_key_exists("sctpconnections", $c);
                }),
              );
              $parts =
                !empty($conn) && !empty($conn[0]["sctpconnections"])
  ...
  private function makeJWT(string $server, string $room)
  {
    ...
    if (!($appId = $this->getAppId($server))) {
      throw new Exception(
        sprintf(_("No App ID found for the server %s"), $server),
      );
    }
    if (Str::isUid($room)) {
      $r = $this->getRoom($room);
    ...
    if (!($appSecret = $this->getAppSecret($server))) {
      throw new Exception(
        sprintf(_("No App Secret found for the server %s"), $server),
      );
    }
