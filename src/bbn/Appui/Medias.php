...
class Medias extends DbCls
{
  ...
  public function __construct(Db $db)
  {
    ...
    $this->taggerInit($this->class_cfg["tables"]["medias_tags"], [
  ...
  {
    $this->userId = defined("BBN_EXTERNAL_USER_ID")
  ...
  public function getFileUrl(string|null $id = null): ?string
  {
    if ($this->dbTraitExists($id)) {
      ...
      if (!$this->fileRoot) {
        $this->setFileRoot(
          Mvc::getPluginUrl("appui-note") . "/media/download/",
        );
      }
  ...
  public function search(
    ...
    $cfg["join"][] = [
      ...
      "on" => [
        [
          "field" => $this->db->cfn(
            $cf["arch"]["medias_tags"]["id_media"],
            $cf["tables"]["medias_tags"],
          ),
          "exp" => $this->db->cfn(
            $cf["arch"]["medias"]["id"],
            $cf["tables"]["medias"],
  ...
  public function browse(array $cfg, int $limit = 20, int $start = 0): ?array
  {
    if ($this->usr) {
      ...
      if (
        ($pvtIdx = X::search($filters, ["field" => $ct["private"]])) === null
      ) {
        $filters[] = [
          "field" => $ct["private"],
          "value" => 0,
      ...
      foreach ($res["data"] as $i => $d) {
        $media_groups_media = $this->db->rselect(
          ...
          [
            ...
            $cf["arch"]["medias_groups_medias"]["id_media"] =>
              $d[$cf["arch"]["medias"]["id"]],
          ],
        );
        $res["data"][$i][$cf["arch"]["medias_groups_medias"]["position"]] =
          $media_groups_media["position"];
        $res["data"][$i][$cf["arch"]["medias_groups_medias"]["link"]] =
          $media_groups_media["link"];
      }
    }
    return $res;
  }
  ...
  public function insert(
    ...
    if (
      ...
      if (empty($title)) {
        $title = trim(
          str_replace(["-", "_", "+"], " ", X::basename($file, ".$ext")),
        );
      }
      ...
      if (
        !$this->db->insert($cf["table"], [
          ...
          $cf["arch"]["medias"]["description"] => normalizer_normalize(
            $description,
          ),
        ...
        throw new Exception(
          X::_("Impossible to insert the media in the database"),
        );
      }

      $id = $this->db->lastId();
      if ($this->fs->createPath($path . $id)) {
        $this->fs->move($file, $path . $id);
        // Normalize the filename
        rename(
          $path . $id . "/" . X::basename($file),
          $path . $id . "/" . $name,
        );
        ...
        if (Str::pos($mime, "image/") === 0) {
          ...
          $ts =
            !empty($tst) && !empty($tst["thumbs"])
              ? \array_map(function ($t) {
                return [
                  empty($t["width"]) ? false : $t["width"],
                  empty($t["height"]) ? false : $t["height"],
                  X::hasProps($t, ["width", "height", "crop"], true)
                    ? true
                    : false,
                ];
              }, $tst["thumbs"])
  ...
  public function getThumbsSizes($media, $exists = true): array
  {
    ...
    foreach ($cfg["thumbs"] as $thumb) {
      ...
      if (
        !$exists ||
        $this->fs->exists($this->getThumbPath($media["file"], $size))
      ) {
        $res[] = $size;
      }
    }
  ...
  protected function getThumbPath(
    ...
    if (
      count($size) &&
      (Str::isInteger($size[0]) || Str::isInteger($size[1] ?? null))
    ) {
      ...
      if ($create && !$exists) {
        ...
        if ($size[0] && $size[0] >= $img->getWidth()) {
          ...
          $img->resize($size[0] ?: null, $size[1] ?: null, $size[2] ?? false)
        ) {
          $img->save($file);
        }
      }
  ...
  public function zip($medias, $dest)
  {
    ...
    if (
      ...
      ($zip = new \ZipArchive()) &&
      ((is_file($dest) && $zip->open($dest, \ZipArchive::OVERWRITE) === true) ||
  ...
  public function updateDb(
    ...
    if (!empty($content)) {
      $fields[$this->class_cfg["arch"]["medias"]["content"]] = json_encode(
        $content,
      );
    }
  ...
  public function updateContent(
    ...
    if ($this->fs->isFile($tmp_path)) {
      ...
      if ($media = $this->getMedia($id_media, true)) {
        ...
        if (
          $this->updateDb($id_media, $newName, $title, [
            "path" => $media["path"],
  ...
  public function replaceContent(string $id, string $file)
  {
    ...
    if (($ext = Str::fileExt($file)) && ($media = $this->getMedia($id, true))) {
      $cf = &$this->class_cfg;
      ...
      if (
        ...
        throw new Exception(
          X::_("Impossible to update the media in the database"),
        );
      }
      ...
      $this->fs->move($file, $root . $path . $id);
  ...
  {
    $groups = !empty($idGroup)
      ? [$idGroup]
      : $this->db->getColumnValues(
        $this->class_cfg["tables"]["medias_groups"],
        $this->class_cfg["arch"]["medias_groups"]["id"],
      );
    $fixed = 0;
    if (!empty($groups)) {
      ...
      foreach ($groups as $group) {
        $medias = $this->db->rselectAll([
          "table" => $mgmTable,
          "fields" => [$idMediaMgm, $idGroupMgm, $positionMgm, $createdM],
          "join" => [
            [
              "table" => $mgTable,
              "on" => [
  ...
  protected function transformMedia(
    ...
    if (!empty($data["content"])) {
      ...
      $data["path"] = empty($data["is_image"])
        ? $this->getFileUrl($data["id"])
        : $this->getImageUrl($data["id"]);
      if ($data["is_image"] && is_file($file)) {
        ...
        if (
          $width ||
          $height ||
          ($crop && $data["dimensions"]["w"] !== $data["dimensions"]["h"])
        ) {
          ...
          if (
            $goodSize &&
            ($tmpFile = $this->getThumbPath($file, $goodSize, true, true))
          ) {
            $data["is_thumb"] = true;
            if ($redirect) {
