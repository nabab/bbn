...
class Imessages extends DbCls
{
  ...
  private function _id_type(): ?string
  {
    if (!self::$id_type) {
      if (
        $id = $this->options->fromCode(
          self::BBN_IMESSAGES,
          self::BBN_NOTES_TYPES,
          self::BBN_NOTES,
          self::BBN_APPUI,
        )
      ) {
  ...
  public function get(string $id_option, string $id_user, $simple = true)
  {
    ...
    $id_group = $this->db->selectOne("bbn_users", "id_group", [
      "id" => $id_user,
    ]);
    $db = &$this->db;
    $fields = array_map(function ($a) use ($db, $cfg) {
      ...
      "join" => [
        [
          ...
          "on" => [
            [
              "field" => $this->db->cfn(
                $cfg["arch"]["imessages"]["id"],
                $cfg["table"],
              ),
              "exp" => $this->db->cfn(
                $cfg["arch"]["users"]["id_imessage"],
                $cfg["tables"]["users"],
              ),
            ],
          ],
