...
class Note extends DbCls
{
  ...
  public function __construct(Db $db, string|null $lang = null)
  {
    ...
    self::optionalInit();
    $this->urlType = "note";
    $this->taggerInit($this->class_cfg["tables"]["notes_tags"], [
      ...
      if (Str::isJson($content)) {
        ...
        foreach ($ct as $n => $c) {
          if (is_string($c) && in_array($n, ["title", "text", "html"])) {
            if (Str::isHTML($c)) {
              $excerpt .=
                Str::html2text($c, Str::pos($c, PHP_EOL) > 0) .
                PHP_EOL .
                PHP_EOL;
            } else {
              $excerpt .= $c;
            ...
            foreach ($c as $k => $v) {
              if (is_string($v)) {
                ...
                $excerpt .=
                  Str::html2text($v, Str::pos($v, PHP_EOL) > 0) .
                  PHP_EOL .
                  PHP_EOL;
              }
            }
  ...
  public function getByType(
    ...
    if (Str::isUid($type) && is_int($limit) && is_int($start)) {
      ...
      foreach ($notes as $note) {
        if (
          $medias = $db->getColumnValues(
            $cf["tables"]["notes_medias"],
            $cf["arch"]["notes_medias"]["id_media"],
            [
              $cf["arch"]["notes_medias"]["id_note"] =>
                $note[$cf["arch"]["versions"]["id_note"]],
            ],
          )
        ) {
          $note["medias"] = [];
          foreach ($medias as $m) {
            if (
              ...
              if (Str::isJson($med[$cf["arch"]["medias"]["content"]])) {
                $med[$cf["arch"]["medias"]["content"]] = json_decode(
                  $med[$cf["arch"]["medias"]["content"]],
                );
              }
              $note["medias"][] = $med;
            }
   ...
   * @return int|null
   */
  public function addMediaToNote(
    string $id_media,
    string $id_note,
    int $default = 0,
  ): ?int {
    if ($this->userId) {
      ...
      return $this->db->insertUpdate($cf["tables"]["notes_medias"], [
        $cf["arch"]["notes_medias"]["id_note"] => $id_note,
  ...
  public function getLastVersionCfg($with_content = false): array
  {
    ...
    $grid_cfg = [
      ...
      "fields" => [
        ...
        "last_reply" =>
          ...
          ")",
        "option_name" => $db->cfn(
          $cfo["arch"]["options"]["text"],
          $cfo["table"],
        ),
        "users" =>
          "GROUP_CONCAT(DISTINCT LOWER(HEX(" .
          $db->cfn(
            $cf["arch"]["versions"]["id_user"],
            $cf["tables"]["versions"],
          ) .
        ...
        [
          ...
          "on" => [
            ...
            "conditions" => [
              [
                "field" =>
                  "first_version." . $cf["arch"]["versions"]["id_note"],
                "operator" => "=",
                "exp" => $db->cfn($cf["arch"]["notes"]["id"], $cf["table"]),
              ],
              [
                "field" =>
                  "first_version." . $cf["arch"]["versions"]["version"],
                "operator" => "=",
                "value" => 1,
              ],
            ],
        ...
        [
          ...
          "on" => [
            ...
            "conditions" => [
              [
                "field" =>
                  "replies_versions." . $cf["arch"]["versions"]["id_note"],
                "operator" => "=",
    ...
    if ($with_content) {
      $grid_cfg["fields"]["content"] = $db->cfn(
        $cf["arch"]["versions"]["content"],
        $cf["tables"]["versions"],
      );
    }
  ...
  public function remove(string $id, $keep = false)
  {
    if (Str::isUid($id)) {
      ...
      if (empty($keep)) {
        ...
        $this->db->delete($cf["tables"]["versions"], [
          $cf["arch"]["versions"]["id_note"] => $id,
        ]);
        return $this->db->delete($cf["table"], [
          $cf["arch"]["notes"]["id"] => $id,
        ]);
      } else {
        return $this->db->update(
          $cf["table"],
          [$cf["arch"]["notes"]["active"] => 0],
          [$cf["arch"]["notes"]["id"] => $id],
        );
      }
    }
  ...
  public function removeVersion(string $id, int $version): bool
  {
    ...
    if (Str::isUid($id)) {
      ...
      if (
        $latest =
          $this->latest($id) &&
          ($res = $this->db->delete($cf["tables"]["versions"], [
            $cf["arch"]["versions"]["id_note"] => $id,
            $cf["arch"]["versions"]["version"] => $version,
          ]))
      ) {
  ...
  public function savePostIt(array $cfg): ?array
  {
    ...
    if (!X::hasProps($cfg, ["bcolor", "fcolor"], true)) {
      throw new Exception(
        X::_("Impossible to create a post-it without setting a color"),
      );
    }
  ...
  public function getPostIt(string $id): ?array
  {
    if ($note = $this->get($id)) {
      if (!Str::isJson($note["content"])) {
        throw new Exception(
          X::_("The content of the post-it should be of type JSON"),
        );
      }
  ...
  public function addFeature(
    ...
    if ($num === 0) {
      $num =
        ((int) $this->db->selectOne($table, "MAX(num)", [
          $cols["id_option"] => $id_option,
        ])) + 1;
    }
    if (
      $id = $this->db->selectOne($table, $cols["id"], [
        $cols["id_option"] => $id_option,
  ...
  public function getFeatureOption(string $id_option): string
  {
    if (!Str::isUid($id_option)) {
      ...
      if (!$id_option) {
        throw new Exception(
          X::_("Impossible to determine the feature %s", $id_option),
        );
      }
    }
  ...
  {
    if (
      !$this->db->count($this->class_cfg["tables"]["notes_events"], [
        $this->class_cfg["arch"]["notes_events"]["id_note"] => $id_note,
        $this->class_cfg["arch"]["notes_events"]["id_event"] => $id_event,
      ])
    ) {
