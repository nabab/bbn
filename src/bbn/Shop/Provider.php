...
class Provider extends DbCls
{
  ...
  protected function providerEmailExists(
    string $id_provider,
    string $email,
  ): ?bool {
    $cfg = $this->getClassCfg();
    if (
      $this->db->rselect($cfg["tables"]["emails"], "email", [
        $cfg["arch"]["emails"]["id_provider"] => $id_provider,
  ...
  public function sendSoldOutEmail(string $id_provider, array $product)
  {
    ...
    if (
      ($emails = $this->getEmails($id_provider)) &&
      ($template = $masksCls->getDefault(
        $this->opt->fromCode("product_sold_out", "masks", "appui"),
      ))
    ) {
  ...
  public function edit($id, array $data): ?string
  {
    ...
    if (X::hasProp($data, "name", true)) {
      if (
        X::hasProp($data, "email", true) &&
        !static::providerEmailExists($id, $data["email"])
      ) {
        $this->addProviderEmail($id, $data["email"]);
      }
      return $this->dbTraitUpdate($id, [
        $dbcfg["arch"]["providers"]["name"] => $data["name"],
        $dbcfg["arch"]["providers"]["cfg"] => $data["cfg"]
          ? json_encode($data["cfg"])
          : null,
      ]);
    }
