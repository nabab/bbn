...
class Product extends DbCls
{
  ...
  public function exists(string $id)
  {
    ...
    return (bool) $this->db->count($cfg["table"], [
      $cfg["arch"]["products"]["id"] => $id,
    ]);
  }
  ...
  public function edit(array $data): int
  {
    ...
    $a = $cfg["arch"]["products"];
    if (
      $this->db->selectOne($cfg["table"], $a["id_note"], [
        $a["id"] => $data["id"],
      ])
    ) {
  ...
  {
    return $this->db->selectOne($this->class_table, $this->fields["price"], [
      $this->fields["id"] => $id,
    ]);
  }
  ...
  {
    return $this->db->selectOne($this->class_table, $this->fields["weight"], [
      $this->fields["id"] => $id,
    ]);
  }
  ...
  {
    return (bool) $this->dbTraitUpdate($id, [
      $this->fields["stock"] => $quantity,
    ]);
  }
  ...
  public function remove(string $id): int
  {
    ...
    if ($total["num"]) {
      throw new Exception(
        _("Impossible to delete a product which has already been sold"),
      );
      $toDelete = false;
    }

    if (
      $id_note = $this->db->selectOne(
        $this->class_table,
        $this->fields["id_note"],
        [$this->fields["id"] => $id],
      )
      ...
      foreach ($carts as $c) {
        if ($toDelete) {
          $this->db->delete("bbn_history_uids", [
            "bbn_uid" => $c[$cartCfg["arch"]["cart_products"]["id"]],
          ]);
        } else {
          $this->db->delete($cartCfg["tables"]["cart_products"], [
            $cartCfg["arch"]["cart_products"]["id"] =>
              $c[$cartCfg["arch"]["cart_products"]["id"]],
          ]);
        }
      }
      ...
      return ($toDelete
        ...
        : $this->deactivate($id)) && $this->cms->delete($id_note);
    }
