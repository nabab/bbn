...
class Cart extends DbCls
{
  ...
  public function getCurrentCartID(): ?string
  {
    ...
    if (
      $idCart = $this->dbTraitSelectOne($this->fields["id"], $where, [
  ...
  public function addProduct(
    ...
    if (!$this->productCls->isActive($idProduct)) {
      throw new \Exception(
        sprintf(_("The product with the id %s is not available"), $idProduct),
      );
    }
    if (empty($idCart) && !($idCart = $this->getCurrentCartID())) {
  ...
  public function removeProduct(
    ...
    if ($p = $this->productExists($idCart, $idProduct)) {
      if (!$quantity || $quantity >= $p[$pFields["quantity"]]) {
        return (bool) $this->db->delete($pTable, [
          $pFields["id"] => $p[$pFields["id"]],
        ]);
      } else {
        return $this->setProductQuantity(
          $idCart,
  ...
  public function getProductsDetail(string $idCart = ""): ?array
  {
    if ($products = $this->getProducts($idCart)) {
      ...
      foreach ($products as $i => $p) {
        $products[$i]["product"] = $prodCls->get(
          $p[$this->class_cfg["arch"]["cart_products"]["id_product"]],
        );
      }
    }
    return $products;
  ...
  public function setClient(string $idClient, string $idCart = ""): bool
  {
    ...
    $idCart = empty($idCart) ? $this->getCurrentCartID() : $idCart;
    if (!empty($idCart)) {
      return $this->dbTraitUpdate($idCart, [
  ...
  public function total(string $idAddress, string $idCart = ""): float
  {
    ...
    $total =
      $this->getProductsAmount($idCart) +
      ($this->shippingCost($idAddress, $idCart) ?: 0);
    return \round($total, 2);
  }
  ...
  public function shippingCost(string $idAddress, string $idCart = ""): ?float
  {
    ...
    return \in_array("disabled", \array_values($detail))
      ? null
      : $detail["total"];
  }
   ...
   * @return array
   */
  public function shippingCostDetail(
    string $idAddress,
    string $idCart = "",
  ): array {
    if (empty($idCart)) {
    ...
    if ($products = $this->getProducts($idCart)) {
      ...
      foreach ($products as $product) {
        if (
          !($provider = $this->productCls->getProvider(
            $product[$pFields["id_product"]],
          ))
        ) {
          throw new \Exception(
            sprintf(
              _("No provider found for the product %s"),
              $product[$pFields["id_product"]],
            ),
          );
        }
        if (!isset($providersCosts[$provider])) {
          $providersCosts[$provider] = $this->getProviderShippingCosts(
            $provider,
            $idAddress,
          );
        }
        ...
        if (
          $weight = $this->productCls->getWeight(
            $product[$pFields["id_product"]],
          )
        ) {
          if (!isset($providersWeight[$provider])) {
            $providersWeight[$provider] =
              $weight * $product[$pFields["quantity"]];
          } else {
            $providersWeight[$provider] +=
              $weight * $product[$pFields["quantity"]];
          }
        } elseif (
          !isset($providersDefaults[$provider]) &&
          !empty($providersCosts[$provider]["default"])
        ) {
          $providersDefaults[$provider] = $providersCosts[$provider]["default"];
          $costs[$provider] = \round(
            $costs[$provider] + $providersCosts[$provider]["default"],
            2,
          );
          $costs["total"] = \round(
            $costs["total"] + $providersCosts[$provider]["default"],
            2,
          );
        }
      }
      foreach ($providersWeight as $p => $pw) {
        ...
        $costs[$p] = \round(
          $costs[$p] + $providersCosts[$p]["prices"][$tmpw],
          2,
        );
        $diff = $pw - $tmpw;
   ...
   * @return float
   */
  public function shippingCostPerProvider(
    string $idProvider,
    string $idAddress,
    string $idCart = "",
  ): ?float {
    $detail = $this->shippingCostDetailPerProvider(
      $idProvider,
      $idAddress,
      $idCart,
    );
   ...
   * @return array
   */
  public function shippingCostDetailPerProvider(
    string $idProvider,
    string $idAddress,
    string $idCart = "",
  ): array {
      ...
      foreach ($products as $product) {
        if (
          ...
          if (
            ...
            $providersDefaults[$provider] =
              $providersCosts[$provider]["default"];
            $costs[$provider] = \round(
              $costs[$provider] + $providersCosts[$provider]["default"],
              2,
            );
  ...
  public function totalShippingWeight(string $idCart = ""): int
  {
    ...
    if ($products = $this->getProducts($idCart)) {
      foreach ($products as $product) {
        $prodWeight = $this->productCls->getWeight(
          $product[$pFields["id_product"]],
        );
  ...
  protected function getProductAmount(string $idProduct, int $quantity): float
  {
    ...
    throw new \Exception(
      sprintf(_("No product found with the ID %s"), $idProduct),
    );
  }
  ...
  protected function getContinentFromAddress(string $idAddress): string
  {
    ...
    if (!($continentCode = $opt->getProp($idCountry, "continent"))) {
      throw new \Exception(
        sprintf(_("Continent code not found for the country %s"), $idCountry),
      );
    }
    if (!($continent = $opt->fromCode($continentCode, "territories"))) {
      throw new \Exception(
        sprintf(_("Continent not found with the code %s"), $continentCode),
      );
    }
    return $continent;
  }
  ...
  protected function getCountryFromAddress(string $idAddress): string
  {
    ...
    if (
      !($idCountry = $addressCls->selectOne($aFields["country"], [
        $aFields["id"] => $idAddress,
      ]))
    ) {
      throw new \Exception(
        sprintf(_("Contry not found for the address %s"), $idAddress),
      );
    }
    return $idCountry;
  }

  protected function getProviderShippingCosts(
    string $provider,
    string $idAddress,
  ): array {
    ...
    $prices = [];
    $pc =
      $providerCls->getShippingCosts(
        $provider,
        $this->getCountryFromAddress($idAddress),
      ) ?:
      $providerCls->getShippingCosts(
        $provider,
        $this->getContinentFromAddress($idAddress),
      );
    if (\is_array($pc)) {
  ...
  public function isPaid(string $idCart): bool
  {
    $sales = new Sales($this->db);
    $salesCfg = $sales->getClassCfg();
  ...
  public function sendAbandonedCartEmail(
    ...
    if (
      ...
      ($template = $masksCls->getDefault(
        $opt->fromCode("abandoned_cart", "masks", "appui"),
      )) &&
      ...
      if (!empty($mailData["products"])) {
        foreach ($mailData["products"] as &$p) {
          if (
            $p["product"]["medias"][0] &&
            file_exists($p["product"]["medias"][0]["full_path"])
          ) {
        ...
        $mailData["total"] =
          "€ " .
          (string) number_format(
            round((float) $this->getProductsAmount($idCart), 2),
            2,
            ",",
            "",
          );
