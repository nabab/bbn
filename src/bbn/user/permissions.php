...
class permissions extends bbn\models\cls\basic
{
    ...
    if (($id_option = $this->_get_id_option($id_option, $type))
      ...
      foreach ($os as $o){
        ...
        if ($this->has($o['id'], $type)) {
          $res[] = bbn\x::merge_arrays($o, $this->pref->get_by_option($o['id']) ?: []);
        }
      }
  ...
  public function update_all($routes)
  {
    ...
    if ($id_permission = $this->get_option_root()) {
      ...
      if ($id_page
          && ( $all = bbn\file\dir::get_tree(
            bbn\mvc::get_app_path().'mvc/public', false, function ($a) {
