...
SQL;
  ...
  public function get_prev_messages(string $id_chat, float $moment = null, int $num = 50, string $id_user = null): ?array
  {
    return $this->_get_messages($id_chat, $moment ?: x::microtime(), '<', $num, $id_user);
  }


  public function get_next_messages(string $id_chat, float $moment = null, int $num = 0, string $id_user = null)
  {
    return $this->_get_messages($id_chat, $moment ?: x::microtime(), '>', $num, $id_user);
  }
  ...
  public function set_last_notification(string $id_chat, string $id_user, float $moment = null): bool
  {
    if (bbn\str::is_uid($id_chat) && bbn\str::is_uid($id_user)) {
      if (\is_null($moment)) {
  ...
  private function _get_messages(string $id_chat, float $moment, string $comparator, int $num = 0, string $id_user = null)
  {
    if ($this->check() && bbn\str::is_uid($id_chat)) {
      ...
      if ($this->is_participant($id_chat, $id_user) && is_dir($dir)) {
      ...
      x::sort_by($res, 'time');
      $id_user  = bbn\str::is_uid($id_user) ? $id_user : $this->user->get_id();
