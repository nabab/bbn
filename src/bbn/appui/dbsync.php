...
class dbsync
{
  ...
  public static function first_call(){
    ...
    if ( (self::$dbs->get_engine() === 'sqlite') && !\in_array(self::$dbs_table, self::$dbs->get_tables()) ){
      self::$dbs->exec(<<<MYSQL
        CREATE TABLE "dbsync" (
...
MYSQL
    ...
    else if ( (self::$dbs->get_engine() === 'mysql') && !\in_array(self::$dbs_table, self::$dbs->get_tables()) ){
      self::$dbs->exec(<<<MYSQL
  ...
  public static function delete_completed(float $start = null)
  {
    ...
    if (!$start
        || !($start = self::$dbs->select_one(self::$dbs_table, 'MIN(chrono)', [
          ['db', 'NOT LIKE', self::$db->get_current()],
          'state' => 0
        ]))
    ) {
      $start = time();
