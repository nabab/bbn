...
class mapperv2 extends bbn\models\cls\cache{
  ...
  private
    /** @var string A prefix used for the mapper tables */
    $prefix,
    /** @var string The name of the database where the mapper tables are */
    $admin_db,
    /** @var string The name of the database where the tables to map are */
    $client_db;
	public 
    $schema = false,
    $auto_update = false;
  ...
  public function __construct(bbn\db $db, $database = '', $prefix='bbn'){
    ...
    if ( ($prefix === false) || bbn\str::check_name($prefix) ){
      ...
      die("There is an error here, the prefix $prefix seems wrong");
    }
  }
  ...
  public function get_default_form_config($table){
    ...
    if ( $this->db && ($full_table = $this->db->tfn($table)) ){
      $cfg = [];
      $cols = $this->db->get_columns($full_table);
      foreach ( $cols as $name => $c ){
        $cfg[] = $this->get_default_field_config($full_table, $name);
      }
      return [
          'attr' => [
            'action' => '.',
          ],
          'table' => $this->db->tsn($table),
          'elements' => $cfg
      ];
    }
    return [];
      ...
      if ( strpos($col['type'], 'enum') === 0 ){
        ...
        // Case where the column is a reference to another
          ...
          /** @todo The SQL is not needed... */
            ...
            $cfg['sql'] = $this->db->get_select($ref['table'], $cols);
