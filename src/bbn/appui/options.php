...
class options
{
  ...
  public function option($id){
    ...
    if ( \bbn\str\text::is_integer($id) ) {
      ...
      if ( isset($opt) || ($opt = $this->native_option($id)) ){
        $this->get_value($opt);
        $this->get_cfg($opt);
        if ( \bbn\str\text::is_integer($opt['id_alias']) ){
  ...
  public function full_options($id = 0, $id_parent = false, $where = [], $order = [], $start = 0, $limit = 2000){
    ...
    if ( \bbn\str\text::is_integer($id = $this->from_code($id, $id_parent)) ){
      $list = $this->items($id);
      if ( is_array($list) ){
        $res = [];
        foreach ($list as $i) {
  ...
  private function _prepare(&$it){
    ...
    if ( isset($it[$c['id_parent']]) &&
      ...
      if ( !isset($it[$c['id_alias']]) || !\bbn\str\text::is_integer($it[$c['id_alias']]) ){
        $it[$c['id_alias']] = null;
      }
  ...
  public function add($it, $force = false){
    ...
    if ( $this->_prepare($it) ){
      ...
      if ( $force &&
        !is_null($c['code']) &&
        \bbn\str\text::is_integer($id = $this->db->select_one($this->cfg['table'], $c['id'], [
      ...
      if ( \bbn\str\text::is_integer($id) && $items ){
        foreach ( $items as $it ){
          $it['id_parent'] = $id;
  ...
  public function duplicate($id, $target, $deep = true, $force = false){
    ...
    if ( \bbn\str\text::is_integer($target) ){
  ...
  public function set_alias($id, $alias){
    return $this->db->update_ignore($this->cfg['table'], [
      $this->cfg['cols']['id_alias'] => \bbn\str\text::is_integer($alias) ? $alias : null
    ], [
      $this->cfg['cols']['id'] => $id
    ]);
  }
  ...
  public function unset_prop($id, $prop){
    if ( \bbn\str\text::is_integer($id) && !empty($prop) && ($o = $this->option($id)) ){
      if ( is_string($prop) ){
        $prop = [$prop];
      }
      foreach ( $prop as $k ) {
