 ...
 *
 * LESS CSS compiler, adapted from http://Lessss.org
 *
...
class Less {
			...
			foreach ($mixins as $mixin) {
				...
				foreach ($this->sortProps($mixin->props) as $subProp) {
					if ($suffix !== null &&
						$subProp[0] == "assign" &&
						is_string($subProp[1]) &&
						substr($subProp, 0, 1) != $this->vPrefix)
					{
						$subProp[2] = array(
							'list', ' ',
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new Less_parser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new Less_parser($this, $name);
...
class Less_parser {
	...
	public function __construct($Less, $sourceName = null) {
	 ...
	 *
	 * Consider the function Less::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by Less::match(), which behaves
	 ...
	 * avoid this, Less::seek() is used to remember and set buffer positions.
	 *
			...
			if (is_null($block->type)) {
				...
				if (!isset($block->args)) {
					foreach ($block->tags as $tag) {
						if (!is_string($tag) || $tag[0] != $this->Less->mPrefix) {
							$hidden = false;
							break;
						}
					}
	...
	protected function fixTags($tags) {
		...
		foreach ($tags as &$tag) {
			if ($tag[0] == $this->Less->vPrefix)
				$tag[0] = $this->Less->mPrefix;
		}
		return $tags;
	}
			...
			while (true) {
				...
				if ($this->string($str)) {
					...
					foreach ($str[2] as &$chunk) {
						$chunk = str_replace($this->Less->parentSelector, "$&$", $chunk);
					}
	...
	protected function variable(&$name) {
		$s = $this->seek();
		if ($this->literal($this->Less->vPrefix, false) &&
			($this->variable($sub) || $this->keyword($name)))
		{
			if (!empty($sub)) {
				$name = array('variable', $sub);
			} else {
				$name = $this->Less->vPrefix.$name;
			}
			return true;
		}
...
class Less_formatter_lessjs extends Less_formatter_classic {
	public $disableSingle = true;
