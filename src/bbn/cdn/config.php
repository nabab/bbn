...
class config extends bbn\models\cls\basic
{
  ...
  protected function setFiles(): self
  {
    ...
    if (bbn\x::has_prop($this->cfg['params'], 'f', true)) {
      $this->cfg['params']['files'] = $this->cfg['params']['f'];
      unset($this->cfg['params']['f']);
    }
    if (bbn\x::has_prop($this->cfg['params'], 'd', true)) {
    ...
    // Preconfigured
    elseif (bbn\x::has_prop($this->cfg['params'], 'id', true)) {
      $res = $this->getPreconfig();
    }
    // List of files
    elseif (bbn\x::has_prop($this->cfg['params'], 'files', true) && is_dir($this->fpath.$this->cfg['url'])) {
      $res = $this->getFiles();
    }
    // Directory content
    // Vue component
    ...
    elseif (bbn\x::has_prop($this->cfg['params'], 'lib', true)) {
